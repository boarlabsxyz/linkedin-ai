...
├── tasks/                        # Comprehensive task definitions written by the `task-define` skill (one .md per task, kebab-case slug)
├── .claude/
...
| `task-define` | Turn a one-line idea into a comprehensive task definition at `./tasks/<slug>.md` (kebab-case slug). Spawns an `Explore` sub-agent to scout the repo, then fills a template with Goal, Context, Scope, Approach, Acceptance criteria, Open questions, References. |
