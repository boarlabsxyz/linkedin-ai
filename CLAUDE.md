...
- **`linkedin-stats`** (uid `kiqz2fk`, `/d/kiqz2fk/linkedin-stats`) — Account view: KPIs, trends, audience demographics, posts-per-month, comments-per-month (count + reactions + impressions), plus a side-by-side correlation row comparing two scatter implementations of "posts published that month" (X) vs "post's latest-snapshot impressions" (Y): a Plotly panel (`ae3e-plotly-panel`, requires plugin install) that computes OLS linear regression in-browser via the script option (slope + Pearson r in the legend), and a native `xychart` panel that renders scatter only (no built-in trendline; uses an `organize` transformation to put `posts_in_month` first so auto-mapping picks it as X). No variables. `posts_per_month` entries include `total_impressions` (sum of each post's latest-snapshot `impressions`) and `avg_impressions_per_post`; both pull from the latest weekly snapshot, so the trailing month reads low until posts accumulate impressions. `correlation_points` (one entry per post: `posts_in_month`, `impressions`, `id`, `posted_date`, `type`) and `correlation_trend` (2 endpoints of the OLS line) feed the scatter panels.
