...
`scripts/` holds one-shot scripts + LaunchAgent plists + a colocated installer for jobs that run on Peter's Mac Studio on a schedule outside the GitHub Actions cron path (GH cron is too unreliable — hourly jobs drop at >75%, see [community#196910](https://github.com/orgs/community/discussions/196910)). Current entry: `slack-heartbeat.sh` (hourly "Heartbeat <UTC>" post to Slack via the claude.ai Slack connector) + `xyz.boarlabs.slack-heartbeat.plist` (`StartCalendarInterval: Minute=0`, `SessionCreate=false` so the process joins the login-window security session and reads the login keychain) + `install-heartbeat.sh` (copies plist to `~/Library/LaunchAgents/`, bootstraps into `gui/$(id -u)`, kickstart-tests). Same auth mechanics as the self-hosted runner — no `CLAUDE_CODE_OAUTH_TOKEN`, keychain OAuth carries `user:mcp_servers`. Logs at `~/Library/Logs/slack-heartbeat.{out,err}.log`. Adding a new hourly job = drop `scripts/<job>.sh` + `scripts/xyz.boarlabs.<job>.plist` + optional installer. Manage via `launchctl print gui/$(id -u)/<label>` and `launchctl kickstart -k gui/$(id -u)/<label>` for on-demand fires.
