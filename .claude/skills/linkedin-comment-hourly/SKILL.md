...
| Profile store | `./dashboards/profiles/` — **one file per person**, shared with `linkedin-stats` (implementation in `.claude/skills/pipeline-shared/profile-store.mjs`, flag `--profiles-dir`). Filename is `<slug>.json`, or `<ascii-fold>--<hash16>.json` when the slug is not plain ASCII. Profile-first: name, headline, link, `first_seen_at` / `updated_at`, the **scraped profile text**, and a nested `icp` block holding the verdict derived from it. A profile read once is **not re-opened for 10 days** (`--icp-cache-ttl-days`), whatever changes about the person. The verdict carries the `rubric_hash` it was judged under: retune `icp-filter.md` or re-sync `sources/icp.md` and every cached author is **re-judged from the stored text, with no page loads**. `linkedin-stats` writes name/headline records into the same store but never opens a profile page, and a cheap headline-only write can never delete text this pipeline paid a page load for. Written by the gather; hand-editable. |
