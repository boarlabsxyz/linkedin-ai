...
//
// The URN-decode test runs against the REAL comments.json corpus: every
// outbound comment already carries both its URN and the timestamp the pipeline
// derived from it months ago, so any drift in the decode shows up immediately.
...
test('commentUrnToMs matches the timestamps already stored in comments.json', () => {
  const file = path.join(REPO_ROOT, 'dashboards', 'li-stats', 'comments.json');
  const corpus = Object.values(JSON.parse(fs.readFileSync(file, 'utf8')).comments ?? {});
  assert.ok(corpus.length > 0, 'comments.json should not be empty');
  let checked = 0;
  for (const c of corpus) {
    const decoded = P.commentUrnToMs(c.comment_urn);
    assert.ok(decoded, `failed to decode ${c.comment_urn}`);
    // Stored value is second-resolution ISO; allow the sub-second remainder.
    assert.ok(Math.abs(decoded - Date.parse(c.commented_at)) < 1000,
      `${c.comment_urn}: decoded ${new Date(decoded).toISOString()} != stored ${c.commented_at}`);
    checked++;
  }
  assert.ok(checked >= 10, `only ${checked} comments checked`);
});
...
test('reply events drop Peter replying to himself', () => {
  ...
  const replies = [
    ...
    { reply_urn: 'urn:li:comment:(activity:7487568033847070720,7491792908463964162)', name: 'Petro', url: 'https://www.linkedin.com/in/ovchyn', headline: '', text: 'thanks' },
  ];
  const r = P.buildReplyEvents({ comment, replies });
  assert.equal(r.events.length, 1);
